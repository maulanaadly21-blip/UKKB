...
const sendSuccess = (res, statusCode = 200, message = 'Berhasil memproses permintaan', data = null) => {
...
const sendError = (res, statusCode = 400, message = 'Terjadi kesalahan', errorName = 'Bad Request') => {
    return res.status(statusCode).json({
        status: false,
        statusCode,
        message,
...
const extractAppKey = (req, res, next) => {
    req.appKey = req.headers['x-maker-key'] || req.headers['x-app-key'] || DEFAULT_APP_KEY;
    next();
};
...
app.post('/api/maker/register', async (req, res) => {
    try {
        ...
        const result = await dbRun(`INSERT INTO app_makers (name, username, email, password, app_key) VALUES (?, ?, ?, ?, ?)`, [name, username, email, hashedPassword, appKey]);
...
app.post('/api/maker/login', async (req, res) => {
    try {
...
app.get('/api/maker/me', authenticateToken, async (req, res) => {
    try {
...
app.get('/api/maker/stats', async (req, res) => {
    try {
...
app.get('/api/maker/list', async (req, res) => {
    try {
...
app.post('/api/auth/register/member', async (req, res) => {
    try {
        ...
        const hashedPassword = await bcrypt.hash(password, 10);
        const userRes = await dbRun(`INSERT INTO users (username, password, role, app_key) VALUES (?, ?, 'member', ?)`, [username, hashedPassword, appKey]);
        const memberRes = await dbRun(`INSERT INTO members (id_user, nama_member, instansi, alamat, telp, foto, app_key) VALUES (?, ?, ?, ?, ?, ?, ?)`, [userRes.lastID, nama_member, instansi || '', alamat || '', telp || '', foto || 'member_john.jpg', appKey]);
        const token = jwt.sign({ id: userRes.lastID, username, role: 'member', member_id: memberRes.lastID, app_key: appKey }, JWT_SECRET, { expiresIn: '7d' });
...
app.post('/api/auth/register/admin-space', async (req, res) => {
    try {
        ...
        const hashedPassword = await bcrypt.hash(password, 10);
        const userRes = await dbRun(`INSERT INTO users (username, password, role, app_key) VALUES (?, ?, 'admin_space', ?)`, [username, hashedPassword, appKey]);
        const ownerRes = await dbRun(`INSERT INTO space_owners (id_user, nama_coworking, nama_pemilik, telp, app_key) VALUES (?, ?, ?, ?, ?)`, [userRes.lastID, nama_coworking, nama_pemilik, telp || '', appKey]);
        const token = jwt.sign({ id: userRes.lastID, username, role: 'admin_space', owner_id: ownerRes.lastID, app_key: appKey }, JWT_SECRET, { expiresIn: '7d' });
    ...
    try {
        ...
        if (!user) {
            ...
            if (maker) {
                ...
                const token = jwt.sign({ id: maker.id, username: maker.username, role: 'app_maker', app_key: maker.app_key }, JWT_SECRET, { expiresIn: '7d' });
                return sendSuccess(res, 200, 'Login App Maker berhasil!', {
                    id: maker.id,
                    username: maker.username,
                    role: 'app_maker',
...
app.get('/api/spaces/types', (req, res) => {
    return sendSuccess(res, 200, 'Berhasil memproses permintaan', [
        {
            tipe: 'desk',
            label: 'Personal Desk',
...
app.get('/api/spaces/availability', async (req, res) => {
    try {
        ...
        // Overlap Check
        const overlap = await dbGet(`SELECT * FROM reservasi 
...
app.get('/api/spaces/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const appKey = req.appKey;
        const s = await dbGet(`SELECT s.*, so.nama_coworking, so.nama_pemilik, so.telp 
       FROM spaces s 
       LEFT JOIN space_owners so ON s.id_owner = so.id 
       WHERE s.id = ? AND s.app_key = ?`, [id, appKey]);
        if (!s) {
...
app.get('/api/diskon/active', async (req, res) => {
    try {
        const appKey = req.appKey;
...
app.post('/api/diskon/check', async (req, res) => {
    try {
...
app.get('/api/diskon/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const appKey = req.appKey;
...
app.post('/api/reservasi', authenticateToken, async (req, res) => {
    try {
       ...
       AND (
        ...
        const result = await dbRun(`INSERT INTO reservasi (
...
app.get('/api/reservasi/my', authenticateToken, async (req, res) => {
    try {
        const appKey = req.appKey;
        let memberId = req.user.member_id;
        if (!memberId) {
...
app.get('/api/reservasi/my/history', authenticateToken, async (req, res) => {
    try {
...
app.get('/api/reservasi/:id/e-ticket', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const appKey = req.appKey;
        const r = await dbGet(`SELECT r.*, s.nama_space, s.tipe, s.harga_per_jam as space_harga,
       ...
       LEFT JOIN space_owners so ON s.id_owner = so.id
       WHERE r.id = ? AND r.app_key = ?`, [id, appKey]);
        if (!r) {
...
app.get('/api/reservasi/:id', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const appKey = req.appKey;
        const r = await dbGet(`SELECT r.*, s.nama_space, s.harga_per_jam as space_harga, m.nama_member, m.telp
       FROM reservasi r
       LEFT JOIN spaces s ON r.id_space = s.id
       LEFT JOIN members m ON r.id_member = m.id
       WHERE r.id = ? AND r.app_key = ?`, [id, appKey]);
...
app.patch('/api/reservasi/:id/cancel', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const appKey = req.appKey;
...
app.get('/api/admin/profile', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const appKey = req.appKey;
...
app.put('/api/admin/profile', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        ...
        if (!profile) {
            ...
            const resOwner = await dbRun(`INSERT INTO space_owners (id_user, nama_coworking, nama_pemilik, telp, app_key) VALUES (?, ?, ?, ?, ?)`, [userId, nama_coworking, nama_pemilik, telp, appKey]);
            profile = { id: resOwner.lastID };
        }
        else {
            await dbRun(`UPDATE space_owners SET nama_coworking = ?, nama_pemilik = ?, telp = ? WHERE id = ?`, [nama_coworking || profile.nama_coworking, nama_pemilik || profile.nama_pemilik, telp || profile.telp, profile.id]);
        }
...
app.get('/api/admin/members', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
...
app.post('/api/admin/members', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const { username, password, nama_member, instansi, alamat, telp, foto } = req.body;
        const appKey = req.appKey;
        if (!username || !password || !nama_member) {
        ...
        const hashedPassword = await bcrypt.hash(password, 10);
        const userRes = await dbRun(`INSERT INTO users (username, password, role, app_key) VALUES (?, ?, 'member', ?)`, [username, hashedPassword, appKey]);
        const memberRes = await dbRun(`INSERT INTO members (id_user, nama_member, instansi, alamat, telp, foto, app_key) VALUES (?, ?, ?, ?, ?, ?, ?)`, [userRes.lastID, nama_member, instansi || '', alamat || '', telp || '', foto || 'budi.jpg', appKey]);
...
app.get('/api/admin/members/:id', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const { id } = req.params;
        const appKey = req.appKey;
...
app.put('/api/admin/members/:id', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        telp = COALESCE(?, telp),
        foto = COALESCE(?, foto)
       WHERE id = ?`, [nama_member, instansi, alamat, telp, foto, id]);
...
app.delete('/api/admin/members/:id', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const { id } = req.params;
        const appKey = req.appKey;
        const member = await dbGet(`SELECT * FROM members WHERE id = ? AND app_key = ?`, [id, appKey]);
...
app.get('/api/admin/spaces', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const appKey = req.appKey;
...
app.post('/api/admin/spaces', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        ...
        const result = await dbRun(`INSERT INTO spaces (id_owner, nama_space, harga_per_jam, tipe, kapasitas, deskripsi, foto, app_key)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [ownerId, nama_space, Number(harga_per_jam), tipe, Number(kapasitas) || 1, deskripsi, foto || 'desk_alpha_01.jpg', appKey]);
...
app.get('/api/admin/spaces/:id', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const { id } = req.params;
        const appKey = req.appKey;
...
app.put('/api/admin/spaces/:id', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const { id } = req.params;
        const { nama_space, harga_per_jam, tipe, kapasitas, deskripsi, foto } = req.body;
        const appKey = req.appKey;
        ...
        foto = COALESCE(?, foto)
       WHERE id = ?`, [nama_space, harga_per_jam, tipe, kapasitas, deskripsi, foto, id]);
...
app.delete('/api/admin/spaces/:id', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const { id } = req.params;
        const appKey = req.appKey;
        const s = await dbGet(`SELECT * FROM spaces WHERE id = ? AND app_key = ?`, [id, appKey]);
...
app.get('/api/admin/diskon', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const appKey = req.appKey;
        const diskons = await dbAll(`SELECT * FROM diskon WHERE app_key = ? ORDER BY id ASC`, [appKey]);
        return sendSuccess(res, 200, 'Berhasil memproses permintaan', diskons);
...
app.post('/api/admin/diskon', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        ...
        const result = await dbRun(`INSERT INTO diskon (nama_diskon, persentase_diskon, tanggal_awal, tanggal_akhir, app_key)
       VALUES (?, ?, ?, ?, ?)`, [nama_diskon.toUpperCase(), Number(persentase_diskon), tanggal_awal, tanggal_akhir, appKey]);
...
app.get('/api/admin/diskon/:id', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const { id } = req.params;
        const appKey = req.appKey;
...
app.put('/api/admin/diskon/:id', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const { id } = req.params;
        const { nama_diskon, persentase_diskon, tanggal_awal, tanggal_akhir } = req.body;
        const appKey = req.appKey;
       ...
       WHERE id = ?`, [nama_diskon ? nama_diskon.toUpperCase() : null, persentase_diskon, tanggal_awal, tanggal_akhir, id]);
...
app.delete('/api/admin/diskon/:id', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const { id } = req.params;
        const appKey = req.appKey;
        const d = await dbGet(`SELECT * FROM diskon WHERE id = ? AND app_key = ?`, [id, appKey]);
...
app.get('/api/admin/reservasi', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
...
app.patch('/api/admin/reservasi/:id/status', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const { id } = req.params;
...
app.post('/api/admin/reservasi/:id/check-in', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const { id } = req.params;
        const appKey = req.appKey;
        const r = await dbGet(`SELECT * FROM reservasi WHERE id = ? AND app_key = ?`, [id, appKey]);
...
app.post('/api/admin/reservasi/:id/check-out', authenticateToken, requireRole('admin_space'), async (req, res) => {
    try {
        const { id } = req.params;
        const appKey = req.appKey;
        const r = await dbGet(`SELECT * FROM reservasi WHERE id = ? AND app_key = ?`, [id, appKey]);
...
const handleMonthlyReport = async (req, res) => {
    try {
        ...
        const reservations = await dbAll(`SELECT r.*, s.tipe 
       FROM reservasi r 
       LEFT JOIN spaces s ON r.id_space = s.id 
       WHERE r.app_key = ? AND r.status != 'dibatalkan'`, [appKey]);
        const filtered = reservations.filter((r) => {
            const d = new Date(r.tanggal_reservasi);
...
app.post('/api/upload/image', uploadGeneral.any(), (req, res) => {
...
app.post('/api/upload/spaces', uploadSpaces.any(), (req, res) => {
    const file = req.files && req.files.length > 0 ? req.files[0] : req.file;
    if (!file) {
...
app.post('/api/upload/members', uploadMembers.any(), (req, res) => {
    const file = req.files && req.files.length > 0 ? req.files[0] : req.file;
    if (!file) {
